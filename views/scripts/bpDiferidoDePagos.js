...
function limpiar_formulario() {
    $("#titulo").text("Campaña BP Diferidos de Pagos");
    $("#titulo1").text("");
    $("#titulo2").text("");
    $('#pnlEncuesta').hide();
    $('#respuesta1').val("");
...
function mostrar_todos() {
    tabla = $('#tblListado').dataTable({
        ...
        "ajax": {
            url: '../ajax/bpDiferidosC.php?action=selectAll',
            type: "get",
            dataType: "json",
            error: function (e) {
...
function mostrar_uno(Id) {
    $.post("../ajax/bpDiferidosC.php?action=selectById", {Id: Id}, function (datos, estado) {
        ...
        if (camp != "") {
            $.ajax({
                type: "GET",
                url: '../ajax/bpDiferidosC.php?action=estatus',
                data: {camp: camp},
                success: function (r) {
                    $('#level1').html(r);
                }
        ...
        if (idC != "") {
            $.ajax({
                type: "GET",
                url: '../ajax/bpDiferidosC.php?action=telefonos',
                data: {idC: idC},
                success: function (r) {
                    $('#fonos').html(r);
                }
...
$("#level1").change(function () {
    ...
    if (level1 != "") {
        $.ajax({
            type: "GET",
            url: '../ajax/bpDiferidosC.php?action=level2',
    ...
    if (text == "CU4" || text == "CU5" || text == "CU6" || text == "CU7"
            || text == "CU8" || text == "CU9" || text1 == "CU10" || text1 == "CU11"
            || text1 == "CU12" || text == "NU1" || text == "NU2") {
        $('#respuesta1').val("");
        $('#respuesta2').val("");
        $('#respuesta3').val("");
        $('#respuesta1').attr('readonly', true);
...
$("#level2").change(function () {
    ...
    if (level1 !== "" && level2 !== "") {
        id = $.ajax({
            type: "GET",
            url: '../ajax/bpDiferidosC.php?action=code',
...
$("#level3").change(function () {
    ...
    if (level1 !== "" && level2 !== "") {
        $.ajax({
            type: "GET",
            url: '../ajax/bpDiferidosC.php?action=code1',
...
$('#btnFonos').click(function () {
    ...
    if (phones != "" && estatusTel != "") {
        var IDC = $("#IDC").val();
        $.ajax({
            url: '../ajax/bpDiferidosC.php?action=updatePhones',
            method: 'POST',
...
function guardar_datos(e) {
    ...
    $.ajax({
        type: "GET",
        url: '../ajax/bpDiferidosC.php?action=validePhone',
        ...
        success: function (v) {
            ...
            if (v == "Almacene un número de teléfono para continuar!") {
                ...
                $.ajax({
                    url: "../ajax/bpDiferidosC.php?action=save",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
