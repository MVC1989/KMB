...
function mostrar_todos() {
    tabla = $('#tblListado').dataTable({
        ...
        "ajax": {
            url: '../ajax/bpEncuestaAhorroProgramado2.php?action=selectAll',
            type: "get",
            dataType: "json",
            error: function (e) {
...
function mostrar_uno(Id) {
    $.post("../ajax/bpEncuestaAhorroProgramado2.php?action=selectById", {Id: Id}, function (datos, estado) {
        ...
        if (camp != "") {
            $.ajax({
                type: "GET",
                url: '../ajax/bpEncuestaAhorroProgramado2.php?action=estatus',
                data: {camp: camp},
                success: function (r) {
                    $('#level1').html(r);
                }
        ...
        if (idC != "") {
            $.ajax({
                type: "GET",
                url: '../ajax/bpEncuestaAhorroProgramado2.php?action=telefonos',
                data: {idC: idC},
                success: function (r) {
                    $('#fonos').html(r);
                }
...
$("#level1").change(function () {
    ...
    if (level1 != "") {
        $.ajax({
            type: "GET",
            url: '../ajax/bpEncuestaAhorroProgramado2.php?action=level2',
...
$("#level2").change(function () {
    ...
    if (level1 !== "" && level2 !== "") {
        id = $.ajax({
            type: "GET",
            url: '../ajax/bpEncuestaAhorroProgramado2.php?action=code',
...
$("#level3").change(function () {
    ...
    if (level1 !== "" && level2 !== "") {
        $.ajax({
            type: "GET",
            url: '../ajax/bpEncuestaAhorroProgramado2.php?action=code1',
...
$('#btnFonos').click(function () {
    ...
    if (phones != "" && estatusTel != "") {
        var IDC = $("#IDC").val();
        $.ajax({
            url: '../ajax/bpEncuestaAhorroProgramado2.php?action=updatePhones',
            method: 'POST',
...
$('#respuesta6').change(function () {
    if ($('#respuesta6').val() == '0' || $('#respuesta6').val() == '1' || $('#respuesta6').val() == '2' || $('#respuesta6').val() == '3'
            || $('#respuesta6').val() == '4' || $('#respuesta6').val() == '5' || $('#respuesta6').val() == '6') {
        $('#pregunta7').val("¿Por qué no lo recomendaría?");
        $('#respuesta7').attr('required', true);
        $('#respuesta7').attr('readonly', false);

    } else if ($('#respuesta6').val() == '7' || $('#respuesta6').val() == '8') {
        $('#pregunta7').val("¿Qué mejoraría al producto para que lo pueda calificar con un 9 o 10?");
        $('#respuesta7').attr('required', true);
        $('#respuesta7').attr('readonly', false);
    } else {
        $('#respuesta7').attr('required', false);
...
function guardar_datos(e) {
    ...
    $.ajax({
        type: "GET",
        url: '../ajax/bpEncuestaAhorroProgramado2.php?action=validePhone',
        ...
        success: function (v) {
            ...
            if (v == "Almacene un número de teléfono para continuar!") {
                ...
                $.ajax({
                    url: "../ajax/bpEncuestaAhorroProgramado2.php?action=save",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
